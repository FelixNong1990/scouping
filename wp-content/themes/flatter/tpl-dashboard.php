							...
							<table class="couponList">
								...
								?>
									...
									<tr>
										...
										<td class="col3 text-center"><?php echo $ad_views; ?></td>
										<td class="col4 text-center"><span style="color:<?php echo $fontcolor; ?>;"><?php echo $post_status_name; ?></span></td>
										<td class="col5 text-center">
											...
											<?php
												...
												if ( $post_status == 'pending' ) {
													...
													echo '<span class="iconfix dashboard-icons">';

													if ( $clpr_options->coupon_edit ) {
														$edit_url = add_query_arg( array( 'aid' => $post->ID ), CLPR_EDIT_URL );
														$edit_img = html( 'i', array( 'class' => 'editOptions icon-pencil', 'title' => __( 'Edit Coupon', APP_TD ) ) );
													...
													$delete_img = html( 'i', array( 'class' => 'editOptions icon-remove', 'title' => __( 'Delete Coupon', APP_TD ) ) );
													echo html( 'a', array( 'href' => $delete_url, 'onclick' => 'return confirm(flatter_params.text_before_delete_coupon);', 'title' => __( 'Delete Coupon', APP_TD ) ), $delete_img ) . ' ';

													$postaction_url = add_query_arg( array( 'aid' => $post->ID, 'action' => $postaction ), CLPR_DASHBOARD_URL );
													$postaction_img = html( 'i', array( 'class' => 'editOptions icon-pause', 'title' => $postalt ) );
												...
												if ( ! in_array( $post_status, array( 'live', 'offline' ) ) ) {
													$delete_url = add_query_arg( array( 'aid' => $post->ID, 'action' => 'delete' ), CLPR_DASHBOARD_URL );
													echo '<br />' . html( 'a', array( 'href' => $delete_url, 'onclick' => 'return confirm(flatter_params.text_before_delete_coupon);', 'title' => __( 'Delete Coupon', APP_TD ) ), __( 'Delete', APP_TD ) );
												}
											?>
										</td>
									</tr>
