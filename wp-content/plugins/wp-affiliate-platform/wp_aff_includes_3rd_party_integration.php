...
function wp_aff_handle_woocommerce_payment($order_id)
{
	...
	$ip_address = get_post_meta( $order_id, 'Customer IP Address', true );
	if(empty($referrer)){
	...
	if($order_status != "completed" && $order_status != "Processing"){
		...
		wp_affiliate_log_debug("Commission for this transaciton will be awarded when you set the order status to completed.",true);
		return;
	}
	
	if(!empty($referrer)){
		$debug_data = "Commission tracking debug data from the WooCommerce plugin:".$referrer."|".$sale_amount."|".$buyer_email."|".$txn_id."|".$ip_address;
...
{
	if (!is_object($order)){$order = new WC_Order($order);}
	...
	$debug_data = "WooCommerce subscripiton payment - Commission tracking debug data: ".$referrer."|".$sale_amount."|".$buyer_email."|".$txn_id."|".$ip_address;
	wp_affiliate_log_debug($debug_data,true);
	if(!empty($referrer)){
		wp_aff_award_commission_unique($referrer,$sale_amount,$txn_id,$item_id,$buyer_email);
	}
...
{	
	...
	if($wp_aff_platform_config->getValue('wp_aff_enable_wpcf7_lead_capture') === '1')
	{
		...
		if(!empty($aff_id)){	
			...
			$cf7->mail["body"] = apply_filters('wp_aff_cf7_lead_capture_email_body_filter', $cf7->mail["body"], $aff_id);
		}
		else{
